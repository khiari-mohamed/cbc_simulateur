...
class TestRunner {
  ...
  async testScenario1() {
    ...
    try {
      ...
      const quoteResponse = await axios.post(`${API_URL}/quotes/generate`, {
  ...
  async testScenario2() {
    ...
    try {
      const bgId = this.guaranteeIds['BG'];
      const incendieEmeutesId = this.guaranteeIds['INCENDIE_EMEUTES'];
      const ptaId = this.guaranteeIds['PERSONNES_TRANSPORTEES'];
      ...
      const payload = {
        ...
        usageId: this.usageIds['PRIVATE_BUSINESS'],
        formulaType: 'STANDARD',
        selectedGuarantees: [bgId, incendieEmeutesId],
        selectedCapitals: {
          [bgId]: 2000,
          [ptaId]: 5000
        }
