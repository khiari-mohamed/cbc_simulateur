...
export class QuotesController {
  ...
  async generate(@Body() data: { simulationId: string; companyId: string }) {
    try {
      ...
      console.error('Error generating quote:', error.message, error.stack);
      throw error;
    }
  }
  ...
  findMine(@Request() req: any) {
    // If gestionnaire or admin, return ALL quotes for stats
    if (req.user.role === Role.GESTIONNAIRE_VALIDATION_ARS || req.user.role === Role.ADMINISTRATEUR_ARS) {
      return this.quotesService.findByUser(req.user.id); // This returns all user's quotes
    }
  ...
  update(
    ...
    // If modificationNote exists, use updateWithNote, otherwise regular update
