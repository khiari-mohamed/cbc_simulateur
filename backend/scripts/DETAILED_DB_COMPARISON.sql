-- ============================================
-- DETAILED DATABASE COMPARISON SCRIPT
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'formula_eligibilities') THEN
    RAISE NOTICE 'Formula eligibility data:';
    PERFORM * FROM (
      SELECT 
        c.name AS company,
        u."nameFr" AS usage,
  ...
  ELSE
    RAISE NOTICE 'Formula eligibility table does not exist';
  END IF;
END $$;
...
SELECT 
  ...
  COUNT(*) as count
FROM simulations
GROUP BY "formulaType", ("dcCapitals" IS NOT NULL), ("acCapitals" IS NOT NULL), "createdAt"::date
ORDER BY "createdAt" DESC, "formulaType"
LIMIT 10;
